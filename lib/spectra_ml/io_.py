...
def load_spectra_metadata(path):
    """
    Load spectra metadata from CSV file generated by the
    scripts.standardize_spectra.run() function.

    Parameters
    ----------
    path: str
        path to spectra metadata file

    Return value
    ------------
    spectra_metadata: DataFrame
        spectra metadata
    """
    # --- Check arguments
    ...
    spectra_metadata = pd.read_csv(path, sep='|', dtype={'spectrum_id': str})
    # --- Return results
